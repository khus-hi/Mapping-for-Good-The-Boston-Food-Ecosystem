#!/usr/bin/env python3
"""Verify grocery parser output shape and normalization invariants."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify parser normalization for grocery stores CSV."
    )
    parser.add_argument("--input", default="data/cleaned_data/grocery_store_locations_clean.csv", help="CSV input file path")
    parser.add_argument(
        "--parser-module",
        default="parsers.ingest_grocery_stores",
...
def validate_row(raw_row: dict[str, Any], actual: dict[str, Any]) -> list[str]:
    ...
    if actual.get("place_type") != "grocery_store":
        errors.append(f"place_type: expected 'grocery_store', got {actual.get('place_type')!r}")
    if actual.get("datatype") != "grocery store":
        errors.append(f"datatype: expected 'grocery store', got {actual.get('datatype')!r}")
    ...
    if not isinstance(sources, list) or len(sources) != 1 or not isinstance(sources[0], dict):
        ...
        source = sources[0]
        if source.get("source_file") != "grocery_store_locations_clean.csv":
            errors.append(
                "sources[0].source_file: expected 'grocery_store_locations_clean.csv', "
        ...
        if not isinstance(raw, dict):
            ...
            for required in ("Store Name", "Address", "City", "State", "Zip", "County", "Store Type"):
                if required not in raw:
                    errors.append(f"sources[0].raw: missing key {required!r}")

    lat = parse_float(raw_row.get("Latitude") or raw_row.get("latitude"))
    lng = parse_float(raw_row.get("Longitude") or raw_row.get("longitude"))
    ...
    if expected_has_coords:
        ...
        if isinstance(geocoding, dict) and geocoding.get("status") != "NOT_REQUESTED":
            errors.append(
                f"geocoding.status: expected 'NOT_REQUESTED', got {geocoding.get('status')!r}"
            )
